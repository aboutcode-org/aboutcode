...
.. image:: data/post_consolidate.png
...
.. image:: data/post_lic_clarity_score.png

..
    [ToDo] Research and Elaborate the attributes

``--license-policy FILE`` Plugin
--------------------------------
...
.. note::
    ...
    Here only the "license_key" is a required field.
...
It whole JSON file is structured as follows, when the ``--summary`` plugin is applied:

.. image:: data/post_summary.png
...
An example scan using the ``--summary-by-facet`` Plugin::
    ...
    ./scancode -clieu --json-pp sample_facet_summary.json samples --summary --facet dev=*.java --facet dev=*.c --summary-by-facet
...
.. image:: data/post_summary_facet.png
...
.. image:: data/post_summary_key.png
...
``--summary-with-details`` Plugin
---------------------------------

The ``--summary`` plugin summarizes license, copyright and other scan information at the codebase
level. Now running the scan with ``--summary --summary-with-details`` plugins creates summaries
